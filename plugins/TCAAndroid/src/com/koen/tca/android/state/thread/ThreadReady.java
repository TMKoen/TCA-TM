...
public class ThreadReady implements IThreadState {
	...
	public void run() {
		...
		if (device.getServerIpAddress() != null
				&& device.getSocketPortNumber() != 0) {

			try {
				androidServerSocket = new ServerSocket(
						device.getSocketPortNumber());
				...
				while (stopThread != true) {
					...
					if (remoteMsg instanceof ActionMessage) {
						...
						// try {
						// // checks if the action name is a valid member of the
						// ActionObjects enum.
						// // the value of this enum name is not important.
						// // If its not a member of this enum, throws an
						// IllegalArgumentException.
						// ActionObjects.valueOf(intentAction.getActionName());
						//
						// ActionRunner actionRunner = ActionRunner.SINGLETON();
						//
						//
						//
						// // creates dynamically a ITestAction object depends
						// on the name of the action.
						// // The classname must be a fully qualified name, so
						// inclusive the package name.
						// try {
						// actionRunner.setAction((ITestAction)
						// Class.forName("com.koen.tca.android.action."+intentAction.getName()).newInstance());
						// } catch (InstantiationException
						// | IllegalAccessException
						// | ClassNotFoundException e) {
						// // should not by happening because the dynamically
						// created class is of type: ITestAction
						// // e.printStackTrace();
						// }
						//
					...
					//
					// case "Expose_Message":
					// // must not be received in the Ready state, so do
					// nothing.
					// break;
