...
public class TCAClient {
	...
	public static void main(String[] args) {
		...
		// the messages(string) that shows the possible parameters on the command line
		...
		final int FIRST_ARG = 0;	// The index for the first argument that is given by the command line
		final int SECOND_ARG = 1; // the index for the second argument that is given by the command line
		...
		if (args.length > 0)
		{
			// Switch to the right method dependent on the first argument from the command line
			...
			case "addscript":
				...
				// Check if the second command line parameter (the script file name) exists.
				if (args.length > 1)
				{
					client.AddScript(args[SECOND_ARG]);
					System.out.println("done.");
				} 
				else
				{
		...
		{
			// No parameters on the command line. Show the possible parameters on the command line
